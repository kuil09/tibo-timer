...
interface Collection {provider:string;checked_at:string;upstream_at:string|null;coverage:string;checkpoint?:Checkpoint;}
interface State {schema_version:1;processed:Record<string,string>;last_success_at:string|null;last_attempt_at:string|null;last_error:string|null;collection?:Collection;}
...
export async function applySync() {
  ...
  state.collection={provider:plan.batch.provider,checked_at:plan.batch.checked_at,upstream_at:plan.batch.upstream_at,coverage:plan.batch.coverage,...(plan.batch.checkpoint?{checkpoint:plan.batch.checkpoint}:{})};
  await writeJson('data/events.json',doc); await writeJson('data/state.json',state);
  await writeJson('.cache/sync-report.json',{at:now,provider:plan.batch.provider,coverage:plan.batch.coverage,posts:plan.posts.length,processed:log,pending:plan.pending.length-log.filter(e=>e.reason!=='pending_budget'&&e.reason!=='pending_cpu').length});
