...
export function validateFeed(value: unknown): Feed {
  ...
  for (const t of f.tweets as Record<string, unknown>[]) {
    if (!t || typeof t.id !== 'string' || !/^\d+$/.test(t.id) || typeof t.text !== 'string' || !instant(t.at) || typeof t.url !== 'string' || !/^https:\/\/x\.com\/[^/]+\/status\/\d+$/.test(t.url)) throw new Error('invalid_post');
