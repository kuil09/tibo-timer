...
import {verifyClaim,verifyClaimPrompt,verificationSchema,VERIFY_CLAIM_VERSION} from '../src/verify-claim.ts';
...
for(const mode of ['pairs','dev','fresh']){
 ...
 for(const row of base.results){
  ...
  const result=await verifyClaim({text:row.text,truncated:row.result.bypassed},row.result,localStageCall(traces,process.env.LLAMA_URL??'http://127.0.0.1:8083'));
...
const summary={total:rows.length,baseline_correct:rows.filter(x=>x.baseline_passed).length,correct:rows.filter(x=>x.passed).length,baseline_false_completed:rows.filter(x=>x.initial.claim==='completed'&&x.expected.claim!=='completed').length,false_completed:rows.filter(x=>x.false_completed).length,remaining_false_scheduled:rows.filter(x=>x.result.claim==='scheduled'&&x.expected.claim!=='scheduled').length,accepted_correct:rows.filter(x=>x.result.verification.accepted&&x.passed).length,accepted_wrong:rows.filter(x=>x.result.verification.accepted&&!x.passed).length,correct_abstained:rows.filter(x=>x.baseline_passed&&x.result.verification.needed&&!x.result.verification.accepted).length,true_completed:rows.filter(x=>x.true_completed).length,retained_completed:rows.filter(x=>x.retained_completed).length,verification_calls:rows.reduce((n,x)=>n+x.traces.length,0),abstentions_added:rows.filter(x=>x.result.verification.needed&&!x.result.verification.accepted).length,elapsed_ms:performance.now()-started,verification_errors:rows.filter(x=>x.traces.some((t:StageTrace)=>t.error)).length};
const report={version:VERIFY_CLAIM_VERSION,mode:'replay_locked_v3_first_stage_then_live_all_claim_verification',release_eligible:false,identity:{model,baseline_files:inputs,prompt:verifyClaimPrompt,schema:verificationSchema([]),temperature:0,seed:42,reasoning:'off'},summary,rows};
await mkdir('docs/evaluation/local-lab/qwen359b-all-verified',{recursive:true});await writeFile('docs/evaluation/local-lab/qwen359b-all-verified/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(summary));
